...
// appendToFile with retry — if two requests hit simultaneously, one will get a
// SHA conflict (409) from GitHub. Retry once with a fresh read to resolve it.
async function appendToFile(filePath, newItem, retries = 2) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (attempt < retries && isConflict) {
        // Brief pause before retry — let the other write settle

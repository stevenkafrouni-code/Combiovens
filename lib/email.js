...
async function sendQuoteForApproval({ quote, lines, customerNote }) {
  ...
  const html = baseTemplate(`
    ...
    <a href="https://www.combiovens.com.au/api/approve/${quote.quoteId}?pw=${process.env.ADMIN_PASSWORD || 'combiovens2026'}&data=${encodeURIComponent(Buffer.from(JSON.stringify(quote)).toString('base64'))}" class="btn">✓ Approve &amp; Send Quote</a>
    <a href="https://www.combiovens.com.au/api/reject/${quote.quoteId}?pw=${process.env.ADMIN_PASSWORD || 'combiovens2026'}" style="margin-left:16px;color:#6b6b6b;font-size:13px">Reject</a>

...
async function sendQuoteForApproval({ quote, lines, customerNote }) {
  ...
  const html = baseTemplate(`
    ...
    <a href="https://www.combiovens.com.au/api/approve?quoteId=${quote.quoteId}&token=${signToken(quote.quoteId,'approve')}&data=${encodeURIComponent(Buffer.from(JSON.stringify(quote)).toString('base64'))}" class="btn">✓ Approve &amp; Send Quote</a>
    <a href="https://www.combiovens.com.au/api/reject?quoteId=${quote.quoteId}&token=${signToken(quote.quoteId,'reject')}" style="margin-left:16px;color:#6b6b6b;font-size:13px">Reject</a>
...
async function sendPaymentConfirmation({ order, referralCode, referralExpiry }) {
  const html = baseTemplate(`
    ...
    <div class="box">
      ...
      <div style="margin-top:8px;font-size:14px;color:#333333">Total paid: <strong>${formatCurrency(order.total * 1.1)} inc GST</strong></div>
    </div>
    ...
    <div class="box box-orange">
      ...
      <p style="margin:0;color:#333333">Know someone who could use us? Give them this code — or use it yourself on your next order. $250 off, no minimum. Valid until <strong>${referralExpiry}</strong>.</p>
      <p style="margin:8px 0 0;font-size:13px;color:#555555">Single use. They enter the code when submitting their quote.</p>
    </div>

    <h2>What happens next</h2>
    <div class="box">
      <p style="margin:0;color:#1a1a1a;font-size:15px;line-height:1.9">
    ...
    <p style="font-size:13px;color:#333333">Any issues — call ${PHONE}. We run on hospo hours, always.</p>
  `);

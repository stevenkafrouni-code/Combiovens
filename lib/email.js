...
<title>${BRAND}</title>
<style>
  body{margin:0;padding:0;background:#0e0e0e;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;color:#e8e4de}
  .wrap{max-width:600px;margin:0 auto;padding:40px 24px}
  .logo{font-size:22px;font-weight:900;letter-spacing:.02em;color:#fafaf8;text-transform:uppercase;margin-bottom:40px}
  .logo span{color:#e85d04}
  h1{font-size:28px;font-weight:900;text-transform:uppercase;color:#fafaf8;margin:0 0 8px}
  h2{font-size:18px;font-weight:700;text-transform:uppercase;color:#fafaf8;margin:32px 0 12px}
  p{font-size:15px;font-weight:300;line-height:1.65;color:#a8a8a8;margin:0 0 16px}
  .table{width:100%;border-collapse:collapse;margin:16px 0}
  .table th{text-align:left;font-size:11px;font-weight:600;letter-spacing:.12em;text-transform:uppercase;color:#6b6b6b;padding:8px 12px;border-bottom:1px solid #242424}
  .table td{padding:12px;font-size:14px;border-bottom:1px solid #1a1a1a;color:#e8e4de}
  .table .num{text-align:right;font-family:monospace}
  .total-row td{font-weight:700;color:#fafaf8;font-size:16px;border-top:2px solid #3a3a3a}
  ...
  .box{background:#1a1a1a;border:1px solid #242424;border-radius:3px;padding:20px 24px;margin:20px 0}
  .box-orange{border-color:rgba(232,93,4,.3);background:rgba(232,93,4,.06)}
  .label{font-size:11px;font-weight:600;letter-spacing:.12em;text-transform:uppercase;color:#6b6b6b;margin-bottom:4px}
  .val{font-size:16px;font-weight:700;color:#fafaf8}
  .orange{color:#e85d04}
  .footer{margin-top:48px;padding-top:24px;border-top:1px solid #1a1a1a;font-size:12px;color:#6b6b6b;line-height:1.7}
  .specs{display:flex;flex-wrap:wrap;gap:6px;margin-top:4px}
  .spec{background:#242424;color:#a8a8a8;font-size:11px;padding:3px 8px;border-radius:2px}
</style>
</head>
<body>
<div class="wrap">
...
async function sendQuoteForApproval({ quote, lines, customerNote }) {
  ...
  const html = baseTemplate(`
    ...
    <div class="box">
      ...
      <div class="val">${quote.customerName} ${quote.businessName ? '— ' + quote.businessName : ''}</div>
    ...
    <a href="https://combiovens.com.au/admin/approve/${quote.quoteId}" class="btn">✓ Approve &amp; Send Quote</a>
    <a href="https://combiovens.com.au/admin/reject/${quote.quoteId}" style="margin-left:16px;color:#6b6b6b;font-size:13px">Reject</a>
...
async function sendQuoteToCustomer({ quote, lines, claudeNote }) {
  const itemRows = lines.map(l => `
    <tr>
      <td>
        <strong style="color:#fafaf8">${l.name}</strong><br>
        <span style="font-size:12px;color:#6b6b6b">${l.sku}</span>
      </td>
      <td class="num">${l.qty}</td>
      <td class="num">${formatCurrency(l.unitPrice)}</td>
      <td class="num">${formatCurrency(l.lineTotal)}</td>
  ...
  const html = baseTemplate(`
    <h1>Your Quote</h1>
    <p>Hi ${quote.customerName.split(' ')[0]},</p>
    ...
    <div class="box" style="margin-bottom:4px">
      ...
      <div style="font-weight:700;color:#fafaf8">${quote.quoteId}</div>
    </div>
    ...
    <div class="box box-orange">
      <div class="label">Ready to go ahead?</div>
      <p style="margin:8px 0 16px;color:#e8e4de">Click below to accept this quote and proceed to payment. Your order will be placed with the supplier the same day.</p>
      <a href="https://combiovens.com.au/accept/${quote.quoteId}" class="btn">Accept Quote &amp; Pay →</a>
    </div>
